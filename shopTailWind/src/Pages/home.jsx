  ...
  return (
    ...
    {currentPosts.map((product,index) => (
      <div key={index} href={product.description} className="group">
        ...
        <p className="mt-1 text-lg font-medium text-gray-900">Left in stock: {product.stock}</p>
      </div>
    ))}
  </div>
      <Pagination totalPosts={items.length} postsPerPage={postsPerPage} setCurrentPage={setCurrentPage} pageSelected={currentPage} />
      </div>
    </div>
  )
}